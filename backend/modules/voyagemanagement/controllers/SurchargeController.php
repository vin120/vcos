...
class SurchargeController extends Controller
{
	...
	public function actionSurcharge_config(){
		$db=\Yii::$app->db;
		/* 删除 */
		...
		if ($del_id!=''){
			...
			$transaction->commit();
			Helper::show_message(\Yii::t('app','Delete success'), Url::toRoute(['surcharge_config']));
	...
	public function actionSurcharge_option(){
		$db=\Yii::$app->db;
		if ($_POST){
            ...
            try{
                ...
                $transaction->commit();
	    		Helper::show_message(\Yii::t('app','Save success'), Url::toRoute(['surcharge_config']));
            }catch(Exception $e){
                $transaction->rollBack();
                Helper::show_message(\Yii::t('app','Save failed'),Url::toRoute(['surcharge_config']));
            }
			}
			else{//有sid就更新数据
				...
				$transaction->commit();
				Helper::show_message(\Yii::t('app','Update success  '), Url::toRoute(['surcharge_config']));
				}catch(Exception $e){
					$transaction->rollBack();
					Helper::show_message(\Yii::t('app','Update fail  '),Url::toRoute(['surcharge_config']));
				}
			}
		}
