...
class CabincategoryController extends Controller
{
	...
	public function actionCabin_category(){
		/* 船舱大类 */
		$db=\Yii::$app->db;
		/* 删除 */
		...
		if ($code!=''){
			...
			try {
				...
				$transaction->commit();
				Helper::show_message(yii::t('app', "Option success"),Url::toRoute(["cabin_category"]));
				//return $this->redirect("travel_agent?massage=success");
			} catch(Exception $e) {
				$transaction->rollBack();
				Helper::show_message(yii::t('app', "Option fail"),Url::toRoute(["cabin_category"]));
	...
	public function actionCabincategory_option(){
		...
		if ($_POST){
            ...
            try{
                ...
                $transaction->rollBack();
                Helper::show_message(yii::t('app', 'Option failed  '),Url::toRoute(['cabin_category']));
            }
			}
	...
	public function actionSettypeclass(){//船舱归类设置分类
		...
		if ($classval==''){
			Helper::show_message(yii::t('app', "Please Select..."),Url::toRoute(['cabin_category','t'=>1]));
			exit;
		}
	...
	for ($i=0;$i<sizeof($classval);$i++){
	   	...
	   	if (sizeof($mydata)>0){
	    	...
	    	$transaction->commit();
	    	Helper::show_message(yii::t('app', "Option Success"),Url::toRoute(['cabin_category','t'=>1]));
	    	}catch(Exception $e){
	    		$transaction->rollBack();
	    		Helper::show_message(yii::t('app', "Option Fail"),Url::toRoute(['cabin_category','t'=>1]));
	    	}
	    }   
