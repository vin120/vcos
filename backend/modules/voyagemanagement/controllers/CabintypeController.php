...
class CabintypeController extends Controller
{
	...
	//graphic 
	public function actionCabin_type_graphic_add(){
		$db = Yii::$app->db;
		...
		if($_POST){
			...
			try{
				...
				$transaction->commit();
				Helper::show_message('Save success  ', Url::toRoute(['cabin_type_edit','code'=>$code['type_code']]));
			}catch(Exception $e){
				$transaction->rollBack();
				Helper::show_message('Save failed  ','#');
			}
	...
	public function actionCabin_type_graphic_edit(){
	...
	public function actionCabin_type_graphic_del(){
		...
		if(isset($_GET['code'])){
			...
			if($count>0){
				Helper::show_message('Delete the success ', Url::toRoute(['cabin_type_edit','code'=>$type_code['type_code']]));
			}else{
				Helper::show_message('Delete failed ');
			}
		}
	...
	public function actionGet_cabin_type_graphic_page(){
		$pag = isset($_GET['pag'])?$_GET['pag']==1?0:($_GET['pag']-1)*2:0;
