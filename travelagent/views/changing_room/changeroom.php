				...
				<input type="hidden" id="pag" value="<?php echo '1';?>"></input>
					...
					<?php if($key<2){?>
						<tr>
							...
							<input type="button" onclick="changeownroom('<?php echo $value['voyage_code'].','.$value['cabin_type_code'].','.$key.','.$value['cabin_name']?>')" value="change">
							</td>
						</tr>
						<?php }?>
					<?php endforeach;?>
...
window.onload = function(){ 
	...
	$("#search").click(function(){
		...
		$("#t").val('1');
		$.ajax({
			 url:"<?php echo Url::toRoute(['get_changeroom_page']);?>",
             type:'post',
	            	...
	            	success:function(data){
	            		...
	            		if(data != 0){
		    	            ...
		    	            if(key<2){
								...
								str += "</select>";
								str +="<input type='button' onclick='changeownroom(\""+data[key]['voyage_code']+","+data[key]['cabin_type_code']+","+key+","+data[key]['cabin_name']+"\")' value='change'>";
								str +="</td>";
		                        str += "</tr>";
		    	            }
	                          });
